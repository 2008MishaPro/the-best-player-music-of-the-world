...
export const PlaylistPage = reatomComponent(() => {
  ...
  return <section className="page"><header className="playlist-hero"><span className="playlist-art"><ListMusic /></span><div><p className="eyebrow">Плейлист</p><h1>{playlist.name}</h1><p>{playlist.description || "Личная подборка"} · {playlist.items.length} треков</p><div className="button-group"><Button onClick={wrap(() => play(false))}><Play fill="currentColor" />Слушать</Button><Button variant="secondary" onClick={wrap(() => play(true))}><Shuffle />Перемешать</Button><Button variant="ghost" onClick={wrap(() => pinPlaylistAction(playlist.id, !playlist.isPinned))}>{playlist.isPinned ? <PinOff /> : <Pin />}{playlist.isPinned ? "Открепить" : "Закрепить"}</Button><Button variant="ghost" onClick={wrap(() => deletePlaylistAction(playlist.id))}><Trash2 /></Button></div></div></header><TrackList tracks={playlist.items.map((item) => item.track)} emptyMessage="В этом плейлисте пока нет треков" /></section>;
}, "PlaylistPage");
